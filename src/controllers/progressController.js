...
const saveProgress = asyncHandler(async (req, res) => {
  ...
  // WICHTIG: Coins/Gems werden NICHT mehr absolut vom Client übernommen, sondern
  // als DELTA (Zugewinn seit dem letzten Sync DIESES Tabs) auf den aktuellen
  // Datenbank-Wert aufaddiert. Das ist der eigentliche Fix für den gemeldeten
  // Bug: ein alter, längst überholter Browser-Tab konnte vorher mit einem
  // veralteten absoluten Wert (z.B. weniger Coins) eine zwischenzeitliche
  // Admin-Korrektur oder einen Sync eines anderen Tabs einfach überschreiben.
  const coinsDelta = Math.max(0, Math.round(Number(req.body.coinsDelta) || 0));
  const gemsDelta = Math.max(0, Math.round(Number(req.body.gemsDelta) || 0));
  ...
  if (
    ...
    user.flagged = true;
    user.flagReason = `Tageslimit überschritten am ${today} (Coins: ${user.dailyEconomy.coinsGained}, Gems: ${user.dailyEconomy.gemsGained})`;
    await user.save();
    logActivity(user, "cheat_flag", { coinsGained: user.dailyEconomy.coinsGained, gemsGained: user.dailyEconomy.gemsGained });
    throw new ApiError(429, "Tageslimit für Coins/Gems erreicht. Dein Account wurde zur Kontrolle markiert.");
  }

  user.progress.coins += coinsDelta;
  user.progress.gems += gemsDelta;
  user.progress.totalCoinsEarned += coinsDelta;
